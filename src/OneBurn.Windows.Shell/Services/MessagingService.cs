...
namespace OneBurn.Windows.Shell.Services
{
    /// <inheritdoc />
    /// <summary>
    ///     The MessagingService is a class allowing objects to exchange messages.
    /// </summary>
    public class MessagingService : IMessagingService
    {
        private static readonly object CreationLock = new object();
        private static IMessagingService _instance;
        ...
        /// <summary>
        ///     Gets the MessagingService's default instance, allowing
        ...
        ///         cause a memory leak if you don't call
        ///         <see cref="M:OneBurn.Windows.Shell.Services.MessagingService.Unregister(System.Object)" /> when you are cleaning
        ///         up.
        ///     </para>
        /// </summary>
        /// <typeparam name="TMessage">
        ...
        /// <summary>
        ///     Provides a way to override the MessagingService.Instance instance with
        ///     a custom instance, for example for unit testing purposes.
        /// </summary>
        /// <param name="newMessagingService">The instance that will be used as MessagingService.Instance.</param>
        public static void OverrideDefault(IMessagingService newMessagingService)
        {
            _instance = newMessagingService;
        }

        /// <summary>
        ///     Sets the MessagingService's default (static) instance to null.
        /// </summary>
        ...
        ///     recipients can be garbage collected even though the MessagingService keeps
        ...
        ///     idle. For this reason, a user of the MessagingService class should use
        ///     <see cref="RequestCleanup" /> instead of forcing one with the
        ///     <see cref="Cleanup" /> method.
        /// </summary>
        public void Cleanup()
        {
            CleanUpLists(_recipientsOfSubclassesAction);
            CleanUpLists(_recipientsStrictAction);
            _isCleanupRegistered = false;
        }

        /// <summary>
        ///     Notifies the MessagingService that the lists of recipients should
        ...
        private static void CleanUpLists(IDictionary<Type, List<WeakActionAndToken>> lists)
        {
            if (lists == null)
            {
                return;
        ...
        private static void SendToList<TMessage>(TMessage message, IEnumerable<WeakActionAndToken> weakActionsAndTokens, Type messageTargetType, object token)
        {
            if (weakActionsAndTokens != null)
            {
                ...
                {
                    if (item.Action is IExecuteWithObject executeAction && item.Action.IsAlive && item.Action.Target != null && (messageTargetType == null || item.Action.Target.GetType() == messageTargetType || messageTargetType.IsInstanceOfType(item.Action.Target)) && (item.Token == null && token == null || item.Token != null && item.Token.Equals(token)))
                    {
        ...
        private static void UnregisterFromLists<TMessage>(object recipient, object token, Action<TMessage> action, Dictionary<Type, List<WeakActionAndToken>> lists)
        {
            ...
            lock (lists)
            {
                foreach (var item in lists[messageType])
                {
                    if (item.Action is WeakAction<TMessage> weakActionCasted && recipient == weakActionCasted.Target && (action == null || action.Method.Name == weakActionCasted.MethodName) && (token == null || token.Equals(item.Token)))
                    {
